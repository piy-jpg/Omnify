...
const SUBTITLE_FORMAT_OPTIONS: { id: SubtitleFormat; label: string; name: string; ext: string; category: 'standard' | 'data' | 'advanced' | 'media'; badge?: string }[] = [
  ...
  { id: 'vtt', label: 'VTT', name: 'WebVTT for HTML5 & YouTube', ext: '.vtt', category: 'standard', badge: 'Web' },
  { id: 'txt', label: 'TXT', name: 'Plain Timed Transcript', ext: '.txt', category: 'standard' },
  { id: 'json', label: 'JSON', name: 'Developer & AI Structured Dataset', ext: '.json', category: 'data', badge: 'AI' },
  { id: 'csv', label: 'CSV', name: 'Spreadsheet Timed Rows', ext: '.csv', category: 'data' },
  { id: 'ass', label: 'ASS', name: 'Advanced SubStation Alpha (Styled)', ext: '.ass', category: 'advanced', badge: 'Stylized' },
  { id: 'sbv', label: 'SBV', name: 'YouTube Direct Subtitles', ext: '.sbv', category: 'advanced' },
...
export const VideoFrameStudioPage: React.FC<VideoFrameStudioPageProps> = ({
  ...
  const [formatCategoryFilter, setFormatCategoryFilter] = useState<'all' | 'standard' | 'data' | 'advanced' | 'media'>('all');
  ...
  const handleDownloadSubtitles = () => {
    if (subtitleFormat === 'mp4') {
      handleBurnInSubtitleSnapshot();
      return;
    }
    const file = generateSubtitlesFile(subtitles, subtitleFormat);
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          {activeToolTab === 'subtitles' && (
            <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
              ...
              {/* 1. Header & Multi-Category Format Selector */}
              <div className="space-y-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      Auto-generate AI speech captions, edit timestamps, customize styling, and export across 10 industry formats.
                    </p>
                  </div>
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all flex items-center gap-1.5 shadow-2xs ${
                        showSubtitleStyleSettings
                          ? 'border-purple-600 bg-purple-50 dark:bg-purple-950 text-purple-600 dark:text-purple-400 ring-2 ring-purple-500/20'
                ...
                {/* Format Category Filter & Format Pills */}
                <div className="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/80 space-y-2">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 flex items-center gap-1">
                      ...
                      <span>Export Format Category:</span>
                    </span>

                    {/* Category Filter Pills */}
                    <div className="flex items-center gap-1 bg-white dark:bg-slate-900 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-semibold">
                      {[
                        { id: 'all', label: 'All (10)' },
                        ...
                        <button
                          key={cat.id}
                          onClick={() => setFormatCategoryFilter(cat.id as any)}
                          className={`px-2 py-0.5 rounded-lg transition-all ${
                            formatCategoryFilter === cat.id
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {SUBTITLE_FORMAT_OPTIONS
                      .filter(fmt => formatCategoryFilter === 'all' || fmt.category === formatCategoryFilter)
                      .map(fmt => {
                        const isSelected = subtitleFormat === fmt.id;
                        return (
                          <button
                            key={fmt.id}
                            onClick={() => setSubtitleFormat(fmt.id)}
                            ...
                            className={`px-2.5 py-1 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer ${
                              isSelected
                                ...
                                : 'bg-white dark:bg-slate-900 hover:bg-purple-50 dark:hover:bg-purple-950/50 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700'
                            }`}
                          >
                            <span>{fmt.label}</span>
                            {fmt.badge && (
                              <span className={`text-[8px] font-mono px-1 py-0.2 rounded-full uppercase ${
                                isSelected ? 'bg-white/20 text-white' : 'bg-purple-100 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300'
                              }`}>
                                {fmt.badge}
                              </span>
                            )}
                  ...
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5 pt-0.5">
                    <span className="font-bold text-purple-600 dark:text-purple-400">
                      Selected: {SUBTITLE_FORMAT_OPTIONS.find(f => f.id === subtitleFormat)?.name} ({SUBTITLE_FORMAT_OPTIONS.find(f => f.id === subtitleFormat)?.ext})
                    </span>
                    <span className="text-slate-300 dark:text-slate-600">•</span>
                    <span>Ready for instant download & sync</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-2.5 flex-wrap p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-800">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    accept=".srt,.vtt,.txt,.json,.csv,.ass,.sbv,.ttml,.lrc"
                    onChange={handleImportSubtitles}
                    className="hidden"
                  />
                  <button
                    onClick={() => subtitleFileInputRef.current?.click()}
                    title="Import .SRT, .VTT, .JSON, .CSV, .LRC, .SBV subtitle files"
                    className="px-2.5 py-1.5 rounded-xl bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-xs font-semibold flex items-center gap-1 shadow-2xs cursor-pointer"
                  >
                    <Upload className="w-3.5 h-3.5" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2">
                ...
                >
                  ...
                  <span>
                    {subtitleFormat === 'mp4'
                      ? 'Burn-in Snapshot (.MP4)'
                      : `Download .${subtitleFormat.toUpperCase()} (${SUBTITLE_FORMAT_OPTIONS.find(f => f.id === subtitleFormat)?.label})`}
                  </span>
                </button>
                ...
                >
                  {copiedTranscript ? (
                    ...
                    <>
                      ...
                      <span>Copy {subtitleFormat.toUpperCase()} Content</span>
                    </>
                  )}
                </button>
            ...
            <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    Extract clean, high-precision audio tracks from video files into MP3, WAV, AAC, FLAC, and OGG formats with customizable bitrates and volume normalization.
                  </p>
                </div>
              </div>

              {/* Audio Extractor Settings */}
              <div className="space-y-4 animate-in fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                  ...
                  <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700 space-y-1.5">
                    ...
                    >
                      ...
                      <option value={128}>128 kbps (Speech & Voice Optimized)</option>
